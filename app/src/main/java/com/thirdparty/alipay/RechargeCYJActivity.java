...
/**
 * 
 * @ClassName: RechargeCYJActivity
 * @Description: 缴纳诚意金 只有50元
 * @author howe
 * @date 2015-2-2 下午9:41:32
 * 
 */
public class RechargeCYJActivity extends BaseActivity {
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case SDK_PAY_FLAG: {
				...
				if (TextUtils.equals(resultStatus, "9000")) {
					...
					if (TextUtils.equals(resultStatus, "8000")) {
						Toast.makeText(RechargeCYJActivity.this, "支付结果确认中",
								Toast.LENGTH_SHORT).show();

					} else {
						// 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
						Toast.makeText(RechargeCYJActivity.this, "支付失败",
								Toast.LENGTH_SHORT).show();
			...
			case SDK_CHECK_FLAG: {
				Toast.makeText(RechargeCYJActivity.this, "检查结果为：" + msg.obj,
						Toast.LENGTH_SHORT).show();
				break;
			}
			default:
	...
	public void init() {
		...
		RadioButton cz_fifty = (RadioButton) findViewById(R.id.cz_fifty);
		cz_fifty.setChecked(true);

		RadioButton cz_hundred = (RadioButton) findViewById(R.id.cz_hundred);
		cz_hundred.setVisibility(View.INVISIBLE);
		RadioButton cz_twohundred = (RadioButton) findViewById(R.id.cz_twohundred);
		cz_twohundred.setVisibility(View.INVISIBLE);
		ImageView bootm_line = (ImageView) findViewById(R.id.bootm_line);
		bootm_line.setVisibility(View.GONE);
					...
					public void onResponse(String response) {
						...
						try {
							...
							if (status == 1) {
								showToast("支付成功！");
								Editor edt = sp.edit();
								edt.putBoolean(userId + "haschengyijin", true);
								edt.commit();
							} else {
								showToast("支付失败！");
							}
	...
	public void pay() {
		// 订单
		String orderInfo = getOrderInfo("充值", "兼职达人缴纳诚意金" + money + "元", money
				+ "");
		...
		Runnable payRunnable = new Runnable() {
			...
			public void run() {
				// 构造PayTask 对象
				PayTask alipay = new PayTask(RechargeCYJActivity.this);
				// 调用支付接口，获取支付结果
	...
	public void check(View v) {
		Runnable checkRunnable = new Runnable() {
			...
			public void run() {
				// 构造PayTask 对象
				PayTask payTask = new PayTask(RechargeCYJActivity.this);
				// 调用查询接口，获取查询结果
	...
	public String getOrderInfo(String subject, String body, String price) {
		...
		// 服务器异步通知页面路径
		orderInfo += "&notify_url=" + "\"" + AliPayAsynNotifyUrl + "\"";
